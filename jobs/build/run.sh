...
if grep -q "opensdn-io/tf-dev-env" ./patchsets-info.json ; then
...
if [[ ${LINUX_DISTR} == 'centos' ]]; then
  ...
  mirror_list+=" rocky9/rocky.repo rocky9/rocky-extras.repo rocky9/rocky-devel.repo rocky9/rocky-addons.repo mirror-base-rocky9.repo"
...
for repofile in $mirror_list_for_build $mirror_list mirror-base-centos7.repo mirror-base-rocky9.repo mirror-docker.repo mirror-docker9.repo mirror-pip.conf mirror-docker-daemon.json ubuntu-sources.list ; do
